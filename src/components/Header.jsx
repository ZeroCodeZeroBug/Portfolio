...
import { Calendar, Clock, Box, Wrench, Star } from 'lucide-react'
...
export default function Header() {
  ...
  useEffect(() => {
    const updateDateTime = () => {
      ...
      setCurrentTime(now.toLocaleTimeString('en-US', timeOptions) + ' IST')
    }
  ...
  useEffect(() => {
    const sections = ['hero', 'projects', 'skillset', 'about']
        ...
        <div className="relative flex items-center justify-between gap-1 sm:gap-2 md:gap-4">
          <div className="flex items-center flex-1 min-w-0 relative">
            <button
              ...
              className="flex items-center gap-1 sm:gap-2 mr-1 sm:mr-2 md:mr-4 hover:opacity-80 transition-opacity whitespace-nowrap flex-shrink-0"
            >
              <Star className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0 text-white" />
              <span className="text-xs sm:text-sm md:text-lg font-mono font-semibold text-white">deyo</span>
            </button>
          ...
          <div className="hidden md:flex items-center gap-3 lg:gap-5 text-xs sm:text-sm flex-shrink-0">
