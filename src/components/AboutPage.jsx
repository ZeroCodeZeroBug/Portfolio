...
import { Send, Gamepad2, Music, Code, Sun, MapPin, Laptop, Mail } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchSpotify = async () => {
      try {
        ...
        if (!accessToken) {
          console.warn('Spotify access token not found. Set VITE_SPOTIFY_ACCESS_TOKEN in your .env file')
          return
        }
    ...
    const fetchVSCode = async () => {
      try {
        ...
        if (!apiKey) {
          console.warn('WakaTime API key not found. Set VITE_WAKATIME_API_KEY in your .env file')
          return
        }
        ...
        const data = await response.json()
        console.log('WakaTime response:', data) // Debug log
        
        if (data.data && data.data.length > 0) {
          // Get the most recent heartbeat (they're sorted by time)
          ...
          // No heartbeats today, try to get current status
        ...
        <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 md:py-10 lg:py-12 flex flex-col justify-center min-h-[300px] sm:min-h-[400px]"> 
          <div className="mb-6 sm:mb-8">
            <h1 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-bold leading-tight translate-y-[10px] pr-2 sm:pr-4 break-words lg:-mx-4">
              Briefly about <span className="italic">me</span>.
            </h1>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6 md:gap-8 mx-0 sm:mx-4">
            ...
            >
              I'm a full-stack developer with a focus on building fast, reliable web platforms. I specialize in frontend 
              development using TS, Vue & Nuxt, and build backends with TS on Bun. I also work with Python, and I'm learning 
              Go while exploring Java and Rust in my spare time. Most of my projects are in the hosting space, where performance, 
              security, and scalability are key. I set up Pterodactyl panels and build automation tools, Discord bots, Minecraft 
              plugins, SaaS apps, and handle system administration. I'd describe myself as a cheerful and optimistic person with 
              tons of ideas and plans in different areas. I'm still in university and constantly working on self-improvement. My main 
