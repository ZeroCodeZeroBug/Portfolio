...
import gitCommitBg from '../assets/gitcommitbg.jpg'

const GITHUB_USERNAME = 'deyoyk'
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (errorMsg.includes('rate limit') || errorMsg.includes('API rate limit')) {
            setError('GitHub API rate limit exceeded. Please add a GitHub token to your .env file (VITE_GITHUB_TOKEN) for higher limits.')
          } else if (errorMsg.includes('requires authentication') || errorMsg.includes('Bad credentials')) {
            setError('GitHub API requires authentication. Please add VITE_GITHUB_TOKEN to your .env file.')
          } else {
        ...
        if (data.errors) {
          ...
          setError(`GitHub API error: ${errorMsg}. ${!GITHUB_TOKEN ? 'Try adding VITE_GITHUB_TOKEN to your .env file.' : ''}`)
  ...
  if (loading) {
    return (
      <div className="relative p-4 sm:p-6 rounded-lg border border-white/10 w-full overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-center bg-no-repeat blur-xl scale-105"
          style={{ backgroundImage: `url(${gitCommitBg})` }}
        />
